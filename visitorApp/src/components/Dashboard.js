...
const Dashboard = ({ navigation: { navigate } }) => (
  <Container>
    ...
    >
      <CompanyInfo title="Welcome" />
      <Card transparent style={styles.cardView}>
        {DashboardData.map(({ path, iconName, name, icon }, idx) => {
          return (
            <View key={idx}>
              <TouchableOpacity
              ...
              >
                {icon === 'iconVl' ? (
                  <IconSimple name={iconName} size={27} color="#078cd6" />
                ) : icon === 'iconViE' ? (
                  <IconEntypo name={iconName} size={27} color="#003f7f" />
                ) : icon === 'iconVeE' ? (
                  <Image source={iconName} style={styles.image} />
                ) : icon === 'iconQR' ? (
                  <IconAnt name={iconName} size={27} color="#000" />
                ) : null}
